...
---
Group Members: Kateryna Mykhaylova, Polina Semenova, Michael Moore<br>
...
<br> **Answer: As you can see from the scatter plot below we can see that there seems to be positive linear correlation between the two variables. We can see that at lower bills we have lower tips and vice verca. We can also see that there are some anomalies in the plot and data. For example there is a dot at ~$7 bill and ~$5 tip which would be a ~70% tip. We can see anomalies on the other ends to. For example on a ~$34 bill there was a ~$1.2 tip which would be a ~3.5%. These anomalies reflect nothing about the data, but everything about the person.**
```{r}
...
<br> **Answer: Based on the scatterplot we can see that smokers tip less and that it would seem males will tip more than females. This is a very loose correlation that is visible. More analysis of the data would be necesarry to say this for certian.** <br>
```{r}
...
<br> **Answer: As we can see the tiprate is essentially the same for everyday. Ironically Saturday is lowest tip rate day when it would intuitively seem like the most.** <br>
```{r}
